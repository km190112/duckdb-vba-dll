...
- **Excel を画面、Access を裏のデータベースとして使ってきたが、限界を感じている人。**
  Access の 2GB 上限、数十万行での集計の遅さ、`.accdb` の破損・排他ロックに悩んでいるなら、
  DuckDB は有力な移行先です。SQL はほぼそのまま通り、Excel が画面である点も変わりません。
- **数十万〜数千万行を Excel で集計したい人。** シートに全件を出すのではなく、
  DuckDB 側で `GROUP BY` して結果だけを受け取る使い方に向いています。
- **CSV / Parquet を集計の材料にしたい人**（管理者 DLL で外部アクセスを許可した場合）。
- **配布先ごとに操作範囲を絞りたい人。** 読み取り専用の DLL を渡せば、
  現場が誤ってデータを壊す事故を防げます。

逆に、**多人数が同時に書き込む**基幹システムには向きません（後述の比較を参照）。

## サーバー不要のデータベース比較（DuckDB / Access / SQLite）

いずれも「サーバーを立てず、1 ファイルで動く」データベースですが、得意分野が違います。

| | **DuckDB**（このプロジェクト） | **Access** | **SQLite** |
|---|---|---|---|
| 種類 | 分析（列指向） | 業務（行指向） | 業務（行指向） |
| 得意 | 大量データの集計・分析 | フォームや帳票つきの小規模業務 | アプリ組み込み・単一書き込み |
| 数百万行の集計 | ◎ 非常に速い | △ 重い | ○ そこそこ |
| 1 行ずつの更新 | △ 苦手 | ◎ 得意 | ◎ 得意 |
| 同時書き込み | ✗ 単一プロセスのみ | △ 数人まで | △ 書き込みは 1 つずつ |
| データ量の上限 | 実質ディスク次第 | **2GB** | 実質ディスク次第 |
| SQL | 標準に近く高機能 | 独自方言が多い | 標準的だが機能は少なめ |
| CSV/Parquet 直読み | ◎ | △ | ✗ |
| Windows 以外 | ○ | ✗ | ◎ |

**ざっくりした使い分け**

- **集計・分析が主目的**で、書き込みは日次バッチや取り込みが中心 → **DuckDB**。
  「Excel を画面、Access を DB」構成からの乗り換え先として最も自然です。
- **入力フォームや帳票を Access で作り込んでいて**、それ自体が業務の中心 → 当面は **Access** のまま。
  DuckDB はフォーム機能を持たないので、画面は引き続き Excel/Access が担当します。
- **多人数が頻繁に同時更新する** → いずれも不向き。PostgreSQL / SQL Server などの
  サーバー型を検討してください。

> **要点**: DuckDB は「みんなで書き込む台帳」ではなく「大量データを高速に読んで集計する道具」です。
> Access の置き換えというより、**Access が遅かった集計・分析の部分を DuckDB に任せる**のが向いています。
...
### `?` バインドは「1 文」まで（パラメータの個数に制限はない）

`QueryP` / `ExecP` に渡せる SQL は 1 文（セミコロンで区切らない 1 つの命令）です。
`?` プレースホルダの**個数に制限はありません**。複合主キーの検索や複数条件も問題なく書けます。

```vba
' 複合主キー（社員番号 + 日付）を ? 2 個で検索 — 問題なく動く
arr = QueryP(h, "SELECT * FROM 勤怠 WHERE 社員番号 = ? AND 日付 = ?", _
             Array(101, "2024-01-15"))

' 3 個でも IN でも BETWEEN でも同様
arr = QueryP(h, "SELECT * FROM 売上 WHERE 部署 = ? AND 金額 BETWEEN ? AND ?", _
             Array("営業部", 1000, 5000))
```

制限しているのは**セミコロンで複数の文を並べること**（例: `SELECT ...; DELETE ...`）だけです。
複数文を許すとどのパラメータがどの文に対応するか曖昧になり、検査した内容と実行内容が
ずれる余地が生まれるためです。1 文ずつ `QueryP` / `ExecP` を呼んでください。

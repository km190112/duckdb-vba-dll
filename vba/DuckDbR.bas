Attribute VB_Name = "DuckDbR"
'==============================================================================
' DuckDbR - DuckDB アクセスモジュール【① 読み取り専用】
'
' 対象 DLL   : duckdb_r.dll
' できること : SELECT のみ
' できないこと: INSERT/UPDATE/DELETE、CREATE/DROP/ALTER、DB 作成
'
' 【重要】このモジュールは 64bit Excel 専用です。
'
' 【このファイルは自動生成されています】
'   直接編集せず vba\DuckDb.template.txt を直してから
'   pwsh -File vba\generate_modules.ps1 を実行してください。
'
' 【使い方】
'   1. このファイルを VBE で「ファイル > ファイルのインポート」する
'   2. duckdb_r.dll を任意のフォルダに置く
'   3. 起動時に一度 DuckDbInit "C:\path\to\dll_folder" を呼ぶ
...
'
' 【3 つのモジュールの関係】
'   DuckDbR / DuckDbRW / DuckDbAdmin は Lib の DLL 名だけが異なります。
'   下位向けに書いたコードは上位モジュールでもそのまま動きます。
'==============================================================================
Option Explicit

Private Const DLL_NAME As String = "duckdb_r.dll"
...
Private Declare PtrSafe Function DackVersionRaw Lib "duckdb_r.dll" Alias "DackVersion" ( _
    ByRef result As Variant) As Long

Private Declare PtrSafe Function DackCapabilitiesRaw Lib "duckdb_r.dll" Alias "DackCapabilities" ( _
    ByRef result As Variant) As Long

Private Declare PtrSafe Function DackOpen Lib "duckdb_r.dll" ( _
    ByVal pathPtr As LongPtr, ByRef result As Variant) As Long

Private Declare PtrSafe Function DackClose Lib "duckdb_r.dll" ( _
    ByVal handle As LongLong, ByRef result As Variant) As Long

Private Declare PtrSafe Function DackQuery Lib "duckdb_r.dll" ( _
    ByVal handle As LongLong, ByVal sqlPtr As LongPtr, ByRef result As Variant) As Long

Private Declare PtrSafe Function DackQueryParams Lib "duckdb_r.dll" ( _
    ByVal handle As LongLong, ByVal sqlPtr As LongPtr, ByRef params As Variant, _
    ByRef result As Variant) As Long

Private Declare PtrSafe Function DackListTables Lib "duckdb_r.dll" ( _
    ByVal handle As LongLong, ByRef result As Variant) As Long

Private Declare PtrSafe Function DackDescribe Lib "duckdb_r.dll" ( _
    ByVal handle As LongLong, ByVal tablePtr As LongPtr, ByRef result As Variant) As Long

Private Declare PtrSafe Function DackExecute Lib "duckdb_r.dll" ( _
    ByVal handle As LongLong, ByVal sqlPtr As LongPtr, ByRef result As Variant) As Long

Private Declare PtrSafe Function DackExecuteParams Lib "duckdb_r.dll" ( _
    ByVal handle As LongLong, ByVal sqlPtr As LongPtr, ByRef params As Variant, _
    ByRef result As Variant) As Long

Private Declare PtrSafe Function DackAppendArray Lib "duckdb_r.dll" ( _
    ByVal handle As LongLong, ByVal tablePtr As LongPtr, ByRef data As Variant, _
    ByRef result As Variant) As Long

Private Declare PtrSafe Function DackBegin Lib "duckdb_r.dll" ( _
    ByVal handle As LongLong, ByRef result As Variant) As Long

Private Declare PtrSafe Function DackCommit Lib "duckdb_r.dll" ( _
    ByVal handle As LongLong, ByRef result As Variant) As Long

Private Declare PtrSafe Function DackRollback Lib "duckdb_r.dll" ( _
    ByVal handle As LongLong, ByRef result As Variant) As Long

Private Declare PtrSafe Function DackCreateDatabase Lib "duckdb_r.dll" ( _
    ByVal pathPtr As LongPtr, ByRef result As Variant) As Long

Private Declare PtrSafe Function DackExecuteDDL Lib "duckdb_r.dll" ( _
    ByVal handle As LongLong, ByVal sqlPtr As LongPtr, ByRef result As Variant) As Long

Private Declare PtrSafe Function DackExportSchema Lib "duckdb_r.dll" ( _
    ByVal handle As LongLong, ByVal formatPtr As LongPtr, ByRef result As Variant) As Long

Private Declare PtrSafe Function DackCheckpoint Lib "duckdb_r.dll" ( _
    ByVal handle As LongLong, ByRef result As Variant) As Long
...
'
'   例: DuckDbInit ThisWorkbook.Path & "\dll"
Public Sub DuckDbInit(ByVal dllFolder As String)
    If Not IsWin64() Then
        Err.Raise ERR_BASE, "DuckDbR", _
    ...
    If Dir$(fullPath) = "" Then
        Err.Raise ERR_BASE, "DuckDbR", "DLL が見つかりません: " & fullPath
    End If
...
'--------------------------------------------------- 書き込み（階層② 以上）----
' この DLL（読み取り専用）で呼ぶとエラーになります。
' 使うには DuckDbRW モジュール + duckdb_rw.dll に切り替えてください。
...
'------------------------------------------------------- 管理（階層③ のみ）----
' この DLL（読み取り専用）で呼ぶとエラーになります。
' 使うには DuckDbAdmin モジュール + duckdb_admin.dll に切り替えてください。
...
Private Sub Check(ByVal rc As Long, ByRef result As Variant, ByVal funcName As String)
    ...
    Err.Raise ERR_BASE + Abs(rc), "DuckDbR." & funcName, msg
End Sub

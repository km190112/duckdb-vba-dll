Attribute VB_Name = "DuckDbAdmin"
'==============================================================================
' DuckDbAdmin - DuckDB アクセスモジュール【③ 管理者】
'
' 対象 DLL   : duckdb_admin.dll
' できること : すべて（DB 作成、テーブル作成/削除、キー設定、スキーマ出力を含む）
' できないこと: （制限なし）
'
...
'   2. duckdb_admin.dll を任意のフォルダに置く
...
Private Const DLL_NAME As String = "duckdb_admin.dll"
...
Private Declare PtrSafe Function DackVersionRaw Lib "duckdb_admin.dll" Alias "DackVersion" ( _
    ByRef result As Variant) As Long

Private Declare PtrSafe Function DackCapabilitiesRaw Lib "duckdb_admin.dll" Alias "DackCapabilities" ( _
    ByRef result As Variant) As Long

Private Declare PtrSafe Function DackOpen Lib "duckdb_admin.dll" ( _
    ByVal pathPtr As LongPtr, ByRef result As Variant) As Long

Private Declare PtrSafe Function DackClose Lib "duckdb_admin.dll" ( _
    ByVal handle As LongLong, ByRef result As Variant) As Long

Private Declare PtrSafe Function DackQuery Lib "duckdb_admin.dll" ( _
    ByVal handle As LongLong, ByVal sqlPtr As LongPtr, ByRef result As Variant) As Long

Private Declare PtrSafe Function DackQueryParams Lib "duckdb_admin.dll" ( _
    ByVal handle As LongLong, ByVal sqlPtr As LongPtr, ByRef params As Variant, _
    ByRef result As Variant) As Long

Private Declare PtrSafe Function DackListTables Lib "duckdb_admin.dll" ( _
    ByVal handle As LongLong, ByRef result As Variant) As Long

Private Declare PtrSafe Function DackDescribe Lib "duckdb_admin.dll" ( _
    ByVal handle As LongLong, ByVal tablePtr As LongPtr, ByRef result As Variant) As Long

Private Declare PtrSafe Function DackExecute Lib "duckdb_admin.dll" ( _
    ByVal handle As LongLong, ByVal sqlPtr As LongPtr, ByRef result As Variant) As Long

Private Declare PtrSafe Function DackExecuteParams Lib "duckdb_admin.dll" ( _
    ByVal handle As LongLong, ByVal sqlPtr As LongPtr, ByRef params As Variant, _
    ByRef result As Variant) As Long

Private Declare PtrSafe Function DackAppendArray Lib "duckdb_admin.dll" ( _
    ByVal handle As LongLong, ByVal tablePtr As LongPtr, ByRef data As Variant, _
    ByRef result As Variant) As Long

Private Declare PtrSafe Function DackBegin Lib "duckdb_admin.dll" ( _
    ByVal handle As LongLong, ByRef result As Variant) As Long

Private Declare PtrSafe Function DackCommit Lib "duckdb_admin.dll" ( _
    ByVal handle As LongLong, ByRef result As Variant) As Long

Private Declare PtrSafe Function DackRollback Lib "duckdb_admin.dll" ( _
    ByVal handle As LongLong, ByRef result As Variant) As Long

Private Declare PtrSafe Function DackCreateDatabase Lib "duckdb_admin.dll" ( _
    ByVal pathPtr As LongPtr, ByRef result As Variant) As Long

Private Declare PtrSafe Function DackExecuteDDL Lib "duckdb_admin.dll" ( _
    ByVal handle As LongLong, ByVal sqlPtr As LongPtr, ByRef result As Variant) As Long

Private Declare PtrSafe Function DackExportSchema Lib "duckdb_admin.dll" ( _
    ByVal handle As LongLong, ByVal formatPtr As LongPtr, ByRef result As Variant) As Long

Private Declare PtrSafe Function DackCheckpoint Lib "duckdb_admin.dll" ( _
    ByVal handle As LongLong, ByRef result As Variant) As Long
...
Public Sub DuckDbInit(ByVal dllFolder As String)
    If Not IsWin64() Then
        Err.Raise ERR_BASE, "DuckDbAdmin", _
    ...
    If Dir$(fullPath) = "" Then
        Err.Raise ERR_BASE, "DuckDbAdmin", "DLL が見つかりません: " & fullPath
    End If
...
Private Sub Check(ByVal rc As Long, ByRef result As Variant, ByVal funcName As String)
    ...
    Err.Raise ERR_BASE + Abs(rc), "DuckDbAdmin." & funcName, msg
End Sub
